...
Config.tsc = function()
  ...
  if not bin then return vim.notify("tsc: not found in node_modules", vim.log.levels.ERROR) end

  vim.notify("tsc: running...", vim.log.levels.INFO)

  vim.system({ bin, "--noEmit", "--pretty", "false" }, { text = true }, vim.schedule_wrap(function(result)
    local items = {}
